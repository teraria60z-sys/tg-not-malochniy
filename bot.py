...
PIECE_RATE_SYSTEM = {
    ...
    'gamer': {'rate_per_escort': 1000, 'rate_per_review': 200, 'safety_bonus': 300}
}
...
def cleanup_previous_messages(chat_id):
...
def safe_send_message(chat_id, text, reply_markup=None, parse_mode=None, cleanup_previous=True):
    try:
        if cleanup_previous:
    ...
    try:
        ...
        if call.data == "not_robot":
            ...
            if is_admin(user_id):
                ...
                session.set_state('waiting_admin_pass')
                safe_send_message(user_id, "🔐 Введите пароль админа:")
        elif call.data == "transfer_admin":
            start_transfer_admin(user_id)
        elif call.data.startswith("app_"):
            handle_application_action_callback(call)
        elif call.data.startswith("user_"):
            handle_user_action_callback(call)
...
def show_manager_contact(user_id):
    try:
...
def handle_review_callback(call):
    try:
        ...
        if call.data.startswith("review_target_"):
            ...
            safe_send_message(user_id, "⭐ *Оцените работу сотрудника:*", reply_markup=markup, parse_mode="Markdown")
            ...
            safe_send_message(user_id, "📝 *Напишите комментарий к отзыву:*\n\nОпишите ваше впечатление о работе сотрудника:", parse_mode="Markdown")
    except Exception as e:
...
def handle_messages(message):
    try:
        ...
        if session.state == 'waiting_admin_pass':
...
def show_admin_panel(user_id):
    try:
        ...
        markup.add(users_btn, applications_btn, reviews_btn, salary_btn, stats_btn, broadcast_btn, transfer_btn, back_btn)
...
def handle_application_action(user_id, action, app_id):
    try:
        cursor = db_connection.cursor()
        ...
        if action == "approve":
            ...
            db_connection.commit()
            safe_send_message(user_id, f"✅ Заявка #{app_id} одобрена!")
            ...
            safe_send_message(user_id, f"❌ Заявка #{app_id} отклонена!")
        
        show_admin_applications(user_id)
    except Exception as e:
        logger.error(f"Ошибка в handle_application_action: {e}")

def handle_review_action(user_id, action, review_id):
    try:
        cursor = db_connection.cursor()
        ...
        if action == "approve":
            ...
            db_connection.commit()
            safe_send_message(user_id, f"✅ Отзыв #{review_id} одобрен!")
        
        elif action == "reject":
            cursor.execute('DELETE FROM reviews WHERE id = ?', (review_id,))
            db_connection.commit()
            safe_send_message(user_id, f"❌ Отзыв #{review_id} удален!")
        
        show_admin_reviews(user_id)
    except Exception as e:
        logger.error(f"Ошибка в handle_review_action: {e}")

def handle_application_action_callback(call):
    try:
        user_id = call.message.chat.id
        data = call.data
        
        if data.startswith("admin_application_approve_"):
            app_id = int(data.split("_")[3])
            handle_application_action(user_id, "approve", app_id)
        elif data.startswith("admin_application_reject_"):
            app_id = int(data.split("_")[3])
            handle_application_action(user_id, "reject", app_id)
    except Exception as e:
        logger.error(f"Ошибка в handle_application_action_callback: {e}")

def handle_user_action_callback(call):
    try:
        user_id = call.message.chat.id
        data = call.data
        
        if data.startswith("admin_user_"):
            user_action = data.split("_")[2]
            target_user_id = int(data.split("_")[3])
            handle_user_action(user_id, user_action, target_user_id)
    except Exception as e:
        logger.error(f"Ошибка в handle_user_action_callback: {e}")
